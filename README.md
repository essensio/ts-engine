...
- **Обход узлов — свёрткой (`fold`).** Чтобы пройти `Expr` или `SemType`, бери
  свёртку — `foldExpr` / `foldSemType`: обработчик на каждый вид, рекурсию в детей
  ведёшь сам, набор видов закрыт по типу (пропущенный вид не компилируется).
  Контекст (приоритет печати, режим) ловится носителем `R = (ctx) => …`. Так «какие
  виды узлов есть» не переписывается в каждом потребителе.
  ```ts
  const label = (e: nodes.Expr): string =>
    foldExpr(e, { Num: (n) => n.text, Ref: (r) => r.name, /* … обработчик на КАЖДЫЙ вид … */ });
  ```
  Где свёртка не ложится — узел диктует РАЗНУЮ логику по виду (как тайпчекер) —
  пиши исчерпывающий `switch` с веткой `never`:
  ```ts
  switch (t.kind) {
  ...
  ```
  И свёртка, и `never`-ветка дают одно: пропущенный вид — ошибка компиляции, а не
  баг в рантайме (в этом и смысл размеченного объединения).
