...
- [`src/writer.ts`](src/writer.ts) — печать AST → текст (инверсия парсера): `writeDecl` · `writeType` · `writeExpression` · `writeLiteral`. Скобки минимальны (та же иерархия приоритетов); `parseX(writeX(ast)) ≡ ast`.
...
```ts
import {
  tokenize, isName, parseDeclaration, parseType, parseLiteral, parseExpression,
  writeDecl, writeType, writeExpression, writeLiteral,
  Env, root, same, nodes,
