...
describe("точный текст: типы и объявления", () => {
  ...
  test("объединение и пустой кортеж", () => {
    assert.equal(writeType(N.TUnion([N.TName("Число"), N.TName("Строка")])), "Число | Строка");
    assert.equal(writeType(N.TRel(N.TUnion([N.TName("Число"), N.TName("Строка")]))), "(Число | Строка)[]");
    assert.equal(writeType(N.TTuple([])), "{}");
    assert.equal(writeType(N.TTuple([], true)), "{#}");
  });
});
...
describe("round-trip parseType(writeType(x)) ≡ x", () => {
  for (const src of [
    ...
    "Число | Строка", "(Число | Строка)[]", "Дата | Пусто", "Число & _ > 0 | Строка",
    "{}", "{#}", '{"order-id": Число}',
  ]) {
    test(src, () => {
