...
//   * обходя `Expr`/`SemType`, бери свёртку `foldExpr`/`foldSemType` (обработчик на
//     каждый вид, набор видов закрыт по типу); где узел диктует разную логику по
//     виду (тайпчекер) — исчерпывающий `switch` с веткой `never`. Оба ловят
//     пропущенный вид компилятором, а не рантаймом.
